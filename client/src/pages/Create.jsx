...
export default function Create() {
  ...
  useEffect(() => {
    if (!user) { toast.error("Login required"); navigate("/login"); }
  }, [user, navigate]);

  const [files, setFiles] = useState([
    { name: "index.html", language: "html", value: `<!DOCTYPE html>\n<html>\n<head>\n  <title>App</title>\n  <link rel="stylesheet" href="style.css">\n</head>\n<body>\n  <h1>Auto-Reloading! ⚡</h1>\n  <p>Type something to see it update automatically.</p>\n  <script type="module" src="script.js"></script>\n</body>\n</html>` },
    { name: "style.css", language: "css", value: "body { font-family: sans-serif; padding: 20px; text-align: center; transition: background 0.5s; }" },
    { name: "script.js", language: "javascript", value: `console.log("Hello from Live Server!");` }
  ]);
  ...
  useEffect(() => {
    const handleMessage = (e) => {
        if (e.data.type === "CONSOLE_LOG") setLogs(prev => [...prev, { type: 'log', message: e.data.message }]);
        if (e.data.type === "CONSOLE_WARN") setLogs(prev => [...prev, { type: 'warn', message: e.data.message }]);
        if (e.data.type === "CONSOLE_ERROR") setLogs(prev => [...prev, { type: 'error', message: e.data.message }]);
    };
  ...
  const handleCreateFile = (val) => {
    let name = val.trim(); if (!name) return; if (name.endsWith("/")) name += ".keep";
    if (files.some(f => f.name === name)) return toast.error("File exists!");
    let lang = "plaintext"; if (name.endsWith(".js")) lang = "javascript"; else if (name.endsWith(".css")) lang = "css"; else if (name.endsWith(".html")) lang = "html";
    setFiles([...files, { name, language: lang, value: "" }]); setIsSaved(false); 
  };
  const handleDeleteFile = (fileName) => {
    setFiles(files.filter(f => !f.name.startsWith(fileName))); if (activeFile && activeFile.name.startsWith(fileName)) setActiveFileIndex(0); setIsSaved(false); 
  };
  const handleRenameFile = (oldName, newName) => {
    setFiles(files.map(f => f.name === oldName ? { ...f, name: newName } : f)); setIsSaved(false); 
  };
  const handleEditorChange = (value) => {
    const updated = [...files]; updated[activeFileIndex].value = value; setFiles(updated); setIsSaved(false); 
  };
  ...
  const handleDownload = async () => {
    const zip = new JSZip(); files.forEach(file => { if (!file.name.endsWith(".keep")) zip.file(file.name, file.value); });
    const content = await zip.generateAsync({ type: "blob" }); saveAs(content, `${title || "project"}.zip`); toast.success("Project Downloaded!");
  };

  const handleSubmit = async () => {
    try { 
        const res = await api.post("/snippets", { userId: user._id, title, files }); 
        setIsSaved(true); toast.success("Project Created!"); navigate(`/edit/${res.data._id}`); 
    } catch (err) { console.error(err); toast.error("Save failed. Check console."); }
  };
  ...
  const resize = (e) => {
    if (isDragging && containerRef.current) {
        const newWidth = ((e.clientX - containerRef.current.getBoundingClientRect().left) / containerRef.current.getBoundingClientRect().width) * 100;
    ...
    <div onMouseMove={resize} onMouseUp={stopResizing} onMouseLeave={stopResizing} style={{ height: "calc(100vh - 60px)", display: "flex", flexDirection: "column", backgroundColor: "#1e1e1e", color: "#ccc", fontFamily: "sans-serif" }}>
      ...
      <div style={{ height: "50px", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 15px", backgroundColor: "#252526", borderBottom: "1px solid #000" }}>
         <div style={{display: "flex", alignItems: "center", gap: "15px"}}>
            ...
            <span style={{fontWeight: "bold", color: "#007acc"}}>NEW PROJECT</span>
         ...
         <div style={{display: "flex", gap: "10px"}}>
             ...
             <button onClick={handleSubmit} style={{ backgroundColor: isSaved ? "#007acc" : "#e6a23c", color: "white", border: "none", padding: "6px 12px", borderRadius: "3px", cursor: "pointer", fontWeight: "bold", display: "flex", alignItems: "center", gap: "5px" }}>{isSaved ? "Save" : "Save ●"}</button>
         </div>
      </div>
